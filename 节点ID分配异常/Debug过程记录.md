...
第二份： 
I (1826) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=5 tx_fail=0 last_tx=381ms_ago | rx=0 last_rx=never | msgs_to_rx=0 
W (1836) HAL_CAN: [CAN] transmitting but no frames received -- check slave power, wiring, 50k baud match, and 120R termination 
第三份： 
I (2876) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=9 tx_fail=0 last_tx=131ms_ago | rx=0 last_rx=never | msgs_to_rx=0 
W (2886) HAL_CAN: [CAN] transmitting but no frames received -- check slave power, wiring, 50k baud match, and 120R termination 
4： 
I (3906) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=11 tx_fail=0 last_tx=531ms_ago | rx=0 last_rx=never | msgs_to_rx=0 
W (3916) HAL_CAN: [CAN] transmitting but no frames received -- check slave power, wiring, 50k baud match, and 120R termination 
5： 
I (4956) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=15 tx_fail=0 last_tx=281ms_ago | rx=0 last_rx=never | msgs_to_rx=0 
W (4966) HAL_CAN: [CAN] transmitting but no frames received -- check slave power, wiring, 50k baud match, and 120R termination 
6： 
I (6006) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=17 tx_fail=0 last_tx=631ms_ago | rx=0 last_rx=never | msgs_to_rx=0 
W (6016) HAL_CAN: [CAN] transmitting but no frames received -- check slave power, wiring, 50k baud match, and 120R termination 
直到第7轮日志收到回复： 
I (7056) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=21 tx_fail=0 last_tx=381ms_ago | rx=3 last_rx=331ms_ago | msgs_to_rx=0 
I (7066) GATEWAY_CTRL: ENUM assigned=1 offer=2: 1=15274c76 
{"motor":"DC", "id":1, "status":"standby", "height":0} 
{"motor":"DC", "id":0, "status":"gateway_alive"}
```
①tx_fail = 0，主机侧发送是成功的。
②第一次发送POLL帧后，只有一个W，对应为从机端的【POLL -> skip】情况
如果正常情况下， 应该是
```
...
```
可以发现在第一对I&W（POLL帧对应）的diag之后 ，还有关于OFFER帧对应的diag输出，即此时是从机有对OFFER帧进行处理和响应输出的。
->
**初步怀疑，在从机侧进行日志输出会影响到帧处理。**


### 调用情况3：
①仅考虑原项目代码情况，但不导入`test_shell.c`&不烧写`TEST_BUILD`时（无日志输出），发现也出现了从机侧无法在上电后第一帧响应OFFER的情况：
```
主机侧日志：
1.第一个cycle
...
```
②以在原项目代码下导入`test_shell.c`&烧写`TEST_BUILD`的日志作为对比：
``` 
1.第一个cycle
I (766) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=1 tx_fail=0 last_tx=44ms_ago | rx=0 last_rx=never | msgs_to_rx=0
W (776) HAL_CAN: [CAN] transmitting but no frames received -- check slave power, wiring, 50k baud match, and 120R termination
...
{"motor":"DC", "id":0, "status":"gateway_alive"}
```
③尝试如果在`输出枚举日志`的同时也烧写`TEST_BUILD&test_shell.c`，主机侧日志和从机侧日志显示正常。
```
1.第一个cycle
--主机侧--
I (776) HAL_CAN: [CAN] state=RUNNING TEC=0 REC=0 | tx_ok=1 tx_fail=0 last_tx=44ms_ago | rx=0 last_rx=never | msgs_to_rx=0
...
```
- 即可以发现情况1&2关于`在从机侧使用串口输出日志会对节点分配造成影响`的猜测不正确，转而考虑`TEST_BUILD`会对该通信过程产生影响。
