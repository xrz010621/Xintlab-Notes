...
>类型：**测试报告** - 测试过程记录与分析和结果记录
>作者：本人记录，note1参考Claude code（参考部分正确性不完全确保）
>日期：2026-6-29至2026-7-1
...
**与`TEST_BUILD`的关系**（仅供参考）：可能是由于TEST固件中的UART阻塞式调用和函数不同方式定义导致的系统时序变动，恰好错开了以上情况。

**修改后结果**：重新尝试`烧写/不烧写TEST固件`，节点均分配正常：①从机端日志正常输出`RX POLL | RX OFFER | TX ACK`的周期循环；②主机端日志在上电后第一周期内即可成功对该唯一从机分配id，正确更新`assigned`值。

### **后续问题**：
（以下内容准确度较模糊）
与他人对接后了解到，该双重释放过程理论来说不应对整机（1主控+6从机）的节点id分配流程造成影响——尽管在该单从机的情况下有出现异常现象，但与最终`issue #32`的根因可能无关。
具体来说有几个方面需要注意：
1.从机上电后会进入tick，而主机会优先进行配置、等待等流程，然后开始发送枚举帧，从机收到枚举帧时冻结tick。 （← 这一部分目前没太明白具体意义，应该还需进一步阅读代码）
2.Issue中的问题出现在从机无法接收到POLL帧，而目前测试里遇见的情况是接收到POLL但呈现未接收到OFFER的现象。
3.关于双重释放的问题，给出的解释是：即使在当前cycle丢弃掉OFFER帧，主机会周期循环发送POLL+OFFER的帧序列，并且从机的工作情况为先认为自己接收当前offer的id，然后以该id与自身uid进行哈希退避算法响应ACK；而主机端会通过ASSIGN进行id分配。所以不应受到影响。 （← 但目前对于该流程理解似乎与该解释有出入，需要进一步分析通信流程）