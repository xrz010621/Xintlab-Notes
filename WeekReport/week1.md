...
**针对issue#32** **偶发枚举(POLL/OFFER)帧未被电机单元正确捕获，节点 ID 分配异常**开展工作。
目前想法是在从机侧打印CAN帧接收情况，通过查看CAN接收日志判断是否出现帧丢失等问题。
...
```
- 在从机侧打印时间戳后会发现`连续多次POLL之间时间差在650左右`，即分别`来自不同的cycle`，而<u>同一cycle中的OFFER帧未被处理</u>（FIFO为空，因为下一次处理的是新的POLL；主机侧日志排除了`未成功发送OFFER`的情况）
...
- Q：单个从机侧上电后能够正常分配，需要进一步连多个从机进行检测。但如果多个从机如果同时查看输出，串口应该不够用？但如果只查看主控侧，难以去定位问题？